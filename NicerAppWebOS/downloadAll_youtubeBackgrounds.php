...
$root = dirname(__FILE__).'/siteMedia/backgrounds/iframe/youtube';
$sidelinedRoot = realpath(dirname(__FILE__).'/siteMedia/backgrounds.offline');
$files = getFilePathList ($root, true, '/.*/', null, array('file'));
...
foreach($files as $idx => $file) {
    ...
    $header2 = '<h2>'.$vidID.'</h2>'.PHP_EOL;
    echo $header2;
    
    $url = 'https://youtube.com/watch?v='.$vidID.'&vq=hd2160';
    $xec = 'youtube-dl --prefer-free-formats -format webm "'.$url2.'"';
    $output = 0; $result = -1;
    exec ($xec, $output, $result);
    $dbg = ['$xec' => $xec, '$output'=>$output,'$result'=>$result];
    var_dump ($dbg);

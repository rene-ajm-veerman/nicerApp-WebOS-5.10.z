        ...
        */
        ...
        setTimeout(function() {
            ...
            na.m.waitForCondition("animate?", function() {
                return t.itemsInitialized;//t.items.length > 2;// && t.winners;
            }, function() {
                //debugger;
                t.create(t);
...
//if (it.name.match(/becoming insane/i)) debugger;
            ...
            */
                ...
                if (!t.showFiles || it.name.substr(it.name.length-4,4)=='.mp3') {
                    it.sPos.x = //Math.round( (
                        (p? p.sPos.x + (1000*p.columnOffsetValue):0)
                        + (it.columnOffsetValue * 1000)

                    //) / divider);
                    it.sPos.y = // Math.round( (
                        (p? p.sPos.y + (1000*p.rowOffsetValue):0)
                        + (it.rowOffsetValue * 1000)
                    //) / divider);
                    it.sPos.z = // Math.round( (
                        (p?(1000*(p.level+1)) + (p.depth*1000) + (p.sPos.z):0)
                        + ((it.level * 1000) +(it.depthOffsetValue * 1000))

                    //) / divider);

                } else if (it.model && p) {
                    it.sPos.x = //Math.round( (
                            (p?p.sPos.x + (1000*p.column) + (1000*p.columnOffsetValue):0)
                        + (it.columnOffsetValue * 1000)
                    //) / divider);
                    it.sPos.y = // Math.round( (
                        (p?p.sPos.y + (1000*p.row) + (1000*p.rowOffsetValue):0)
                        + (it.rowOffsetValue * 1000)
                    //) / divider);
                    it.sPos.z = //Math.round(
                        -1 * (
                            (p?p.sPos.z + (p.depth*1000) + (1000*(p.level+1)):0)
                            + ((it.level * 1000) +(it.depthOffsetValue * 1000))
                        )
                    //) / divider;
        ...
        for (var j=0; j<t.items.length; j++) {
            ...
            if (t.ld3[p7a]) {
                var
                color = t.ld3[p7a].color,
                list = t.ld3[p7a].colorList,
                p1 = t.ld3[p7a].p1,
                it = t.items[j];
                if (it) {

class naVividMenu__behavior_rainbowPanels {
    constructor(el, callback7) {
        ...
        t.percentageFor_rainbowPanels = 100;
    ...
    onmouseout (event) {
        ...
        $('.vividMenu_item, .vividMenu_subMenuPanel', t.el).each(function(idx,div) {
            ...
            it = t.items[idx];
            if (it.level === 1) rootLevel.push (div);
        });
        ...
        */
        ...
        //if (toHide.length > 0)
        if (t.useFading) {
            $('.vividMenu_item, .vividMenu_subMenuPanel', t.el).not(rootLevel).not(currs).not(myPeers).stop(true,true).fadeOut(t.fadingSpeed);
            if (t.prevEl.it.label=='4K') debugger;
            debugger;

            //$(currs).not(prevs).stop(true,true).fadeIn(t.fadingSpeed);
        } else {
            $('.vividMenu_item, .vividMenu_subMenuPanel', t.el).not(rootLevel).not(currs).css({display:'none'});

            $(currs).css({display:'block'});
        }
...
//debugger;
        ...
        if (t.type==='horizontal') {
            ...
            typeHorizontal_level_verticalPosition_offsetY = $(it.b.el).height() * (row + 1),
...
/*
        var itemsLabels = t.getIndexesLabels(t, items_final);
debugger;

        if (items_final.length > 0) {
            for (var i=0; i < items_final.length; i++) {
                var
                idx = parseInt(items_final[i]),
                it2 = t.items[idx],
                itp = it.parents && it.parents.length > 0 ? t.items[it.parents[0].idx] : null,
                kids = t.children[idx]; // sub-sub-menus need to get cleared off the field too.
debugger;
                if (it2.b) toBeHidden.push(it2.b.el);

                if (kids)
                for (var kidIdx in kids) {

                    kidIdx = parseInt(kidIdx);
                    var
                    it3 = kids[kidIdx];
                    if (it3.b) toBeHidden.push (it3.b.el);
                }

                if (itp && itp.b)
                    toBeHidden = toBeHidden.remove(itp.b.el);

                var
                panelID = '#'+t.el.id+'__panel__'+idx,
                panel = $(panelID)[0];
                if (panel) toBeHidden.push (panel);
            }
        ...
        t.items_currentEl = items_currentEl;
        t.items_prevEl = items_prevEl;
        t.items_final = items_final;
...
*/
    ...
    showPanel (t, panel, it, pit, dim /* dimensions */, numColumns, numRows, offsetX, offsetY) {
        ...
        /*
        for (var elIdx in t.timeout_hideSubMenu) {
            var to = t.timeout_hideSubMenu[elIdx];
            clearTimeout(to);
        }
        t.timeout_hideSubMenu = [];
        */
        ...
        background2a1 = ''+(Math.random()*255)+','+(Math.random()*255)+','+(Math.random()*255),
    ...
    showBackPanel (t) {
        // show backpanel
        var html = '<div id="'+t.el.id+'__backPanel" class="vividMenu_backPanel">&nsbp;</div>';
        ...
        $(bp).bind('mouseover', function (event) {
            ...
            t.timeout_hideAll[bp.id].push( setTimeout(function (t, bp) {
                ...
                if (t.useFading) $(hiding).stop(true,true).fadeOut(t.fadingSpeed); else $(hiding).css({display:'none'});
                if (t.useFading) $('.vividMenu_subMenuPanel', t.el).stop(true,true).fadeOut(t.fadingSpeed);
                    else $('.vividMenu_subMenuPanel', t.el).css({display:'none'});
                $(bp).remove();
                t.shownChildren = {};
            }, 500, t, bp));
        });

...
?>
        ...
        <?php
                    ...
                    foreach ($filez as $fn=>$fl) {
                        ...
                        echo "\t\t\t".'<audio id="audioTag_'.$idx.'"  onplay="na.m.addLogEntry(\'naMP3 : Now playing : '.$view['rp'].$fn.'\');$(\'#audioTag_'.$idx.'\')[0].volume = 1; var timeInSeconds = Math.floor($(\'#audioTag_'.$idx.'\')[0].currentTime); $(\'#visualizer\')[0].src=\''.$appFolder.'/beatPulse-0.1.0/index3.php?file=/NicerAppWebOS/apps/NicerAppWebOS/applications/2D/musicPlayer.javascriptRendering/music'.$view['rp'].$fn.'&time=\'+timeInSeconds; $(\'.mp3Div\').removeClass(\'playing\');$(event.currentTarget.parentNode).removeClass(\'paused\').addClass(\'playing\');" onended="na.m.addLogEntry(\'naMP3 : Now ended : '.$view['rp'].$fn.'\');$(event.currentTarget.parentNode).removeClass(\'playing\').removeClass(\'paused\').addClass(\'ended\')" onpause="na.m.addLogEntry(\'naMP3 : Now pausing : '.$view['rp'].$fn.'\');$(event.currentTarget.parentNode).removeClass(\'playing\').removeClass(\'ended\').addClass(\'paused\')" onunpause="na.m.addLogEntry(\'naMP3 : Now unpausing : '.$view['rp'].$fn.'\');$(event.currentTarget.parentNode).removeClass(\'paused\').removeClass(\'ended\').addClass(\'playing\')" style="display:flex;order:2;align-items:center;margin:10px;opacity:0.68" onseekwed="$(\'#audioTag_'.$idx.'\')[0].volume = 0.007; var timeInSeconds = Math.floor($(\'#audioTag_'.$idx.'\')[0].currentTime); $(\'#visualizer\')[0].src=\''.$appFolder.'/beatPulse-0.1.0/index3.php?file=/NicerAppWebOS/apps/NicerAppWebOS/applications/2D/musicPlayer.javascriptRendering/music'.$view['rp'].$fn.'&time=\'+timeInSeconds;" controls>'.PHP_EOL;

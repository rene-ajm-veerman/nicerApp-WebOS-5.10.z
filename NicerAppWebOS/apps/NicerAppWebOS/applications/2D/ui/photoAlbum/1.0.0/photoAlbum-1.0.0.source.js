seductiveapps.photoAlbum = sa.pa = {
	...
	init : function (settings) {
		sa.pa.settings.perDiv[settings.iframe] = settings;
		if (settings.image) {
			sa.pa.lowlevel.ui.init (settings);
			jQuery('#'+settings.iframe).addClass ('saPhotoAlbum_iframe');
			sa.m.hookScrollwheel ($('#'+settings.iframe)[0], sa.pa.lowlevel.mousewheel, false, true);
		}
	...
	printAlbum : function (settings) {
		...
		var
		s = sa.pa.settings.perDiv[settings.iframe],
	...
	printAlbum_handleFileLoad : function (event) {
		...
		div.children('a').click (function () { sa.pa.lowlevel.click(this) } );
	...
	setImageDimensions : function (settings) {
		...
		if (!jQueryImg[0].loadHandler) {
			...
			jQueryImg.load ({settings:settings}, function(evt, settings) {
				...
				var
				s = sa.pa.settings.perDiv[evt.data.settings.iframe],
	...
	lowlevel : {
		ui  : {
			...
			update : function (settings) {
				...
				//debugger;
				if (settings) {
					sa.pa.lowlevel.ui.update_zoomOfImage (settings);

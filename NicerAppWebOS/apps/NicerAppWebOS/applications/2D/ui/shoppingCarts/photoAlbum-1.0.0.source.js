seductiveapps.shoppingCartsPhotoAlbum =  sa.scpa = {
	about : {
	...
	init : function (settings) {
		sa.scpa.settings.perDiv[settings.iframe] = settings;
		if (settings.image) {
			sa.scpa.lowlevel.ui.init (settings);
			jQuery('#'+settings.iframe).addClass ('saPhotoAlbum_iframe');
			sa.m.hookScrollwheel ($('#'+settings.iframe)[0], sa.scpa.lowlevel.mousewheel, false, true);
		}
		$(window).resize(sa.scpa.resize);
	},
	...
	resize : function () {
		if (
			...
			for (var div in sa.scpa.settings.perDiv) {
				sa.scpa.printAlbum(sa.scpa.settings.perDiv[div]);
			}		
		}
	},
	...
	printAlbum : function (settings) {
		...
		var
		s = sa.scpa.settings.perDiv[settings.iframe],
		...
		if (sa.scpa.settings.preload) sa.scpa.settings.preload.close();
		sa.scpa.settings.preload = new createjs.LoadQueue (settings.rootURL);
		settings.loadQueue = [];
		sa.scpa.settings.current = settings;
		...
		var
		preload = sa.scpa.settings.preload,
		...
		preload.on("fileload", sa.scpa.printAlbum_handleFileLoad);
        preload.on("progress", sa.scpa.printAlbum_handleOverallProgress);
        preload.on("fileprogress", sa.scpa.printAlbum_handleFileProgress);
        preload.on("error", sa.scpa.printAlbum_handleFileError);
		...
		if (queue.length>0) preload.loadManifest (queue);//sa.scpa.printAlbum_loadAll (settings, queue);
		
	},

	toggleMode : function () {
		if (sa.scpa.settings.mode=='select') {
			sa.scpa.setMode ('normal');
		} else {
			sa.scpa.setMode ('select');
			
		}
	},
	
	setMode : function (newMode) { 
		sa.scpa.settings.mode = newMode;
		if (newMode=='select') {
	...
	printAlbum_handleFileLoad : function (event) {
		...
		div.children('a').click (function () { sa.scpa.lowlevel.click(this) } );
		
		div.children('a').attr('href', sa.m.globals.urls.app + 'photo/' + event.item.id.replace(sa.m.globals.urls.app+'siteData/seductiveapps/com/ui/photoAlbum/albums/','').replace('/lowres','').replace(/\//g,'_') + '--' + s.page);
	...
	printAlbum_handleOverallProgress : function (event) {
		jQuery("#saPhotoAlbum_overallProgress > .saPhotoAlbum_overallProgress").width(sa.scpa.settings.preload.progress * $("#saPhotoAlbum_overallProgress").width());
		if (sa.scpa.settings.preload.progress==1) {
			jQuery('#saPhotoAlbum_overallProgress').fadeOut('normal');
			sa.scpa.setPageButtonLinks(sa.scpa.settings.current);
		};
	...
	checkPageImagesLoaded : function (settings) {
		var
		s = sa.photoAlbum.settings.perDiv[settings.iframe],
		album = settings.db.albumsContents[settings.album],
	...
	setImageButtonLinks : function (settings) {
		...
		jQuery('#btnUp').attr('href', sa.m.globals.urls.app + prefix + 'album/' + album.replace(/\//g,'_') + '--' + settings.page);
		if (urlRelativePrevious) jQuery('#btnPrevious').attr('href', sa.m.globals.urls.app + prefix + 'photo/' + urlRelativePrevious); else jQuery('#btnPrevious').hide();
		if (urlRelativeNext) jQuery('#btnNext').attr('href', sa.m.globals.urls.app + prefix + 'photo/' + urlRelativeNext); else jQuery('#btnNext').hide();
	...
	setImageDimensions : function (settings) {
		...
		if (!jQueryImg[0].loadHandler) {
			...
			jQueryImg.load ({settings:settings}, function(evt, settings) {
				...
				var
				s = sa.scpa.settings.perDiv[evt.data.settings.iframe],
	...
	lowlevel : {
		ui  : {
			...
			update : function (settings) {
				...
				//debugger;
				if (settings) {
					sa.scpa.lowlevel.ui.update_zoomOfImage (settings);
			...
			update_zoomOfImage : function (settings) {
				...
				jQueryImg.css (pos2);
				//debugger;
				sa.sp.containerSizeChanged(jQuery('#iframeContent')[0],true, true); 
				sa.sp.scrollToPos(jQuery('#iframeContent')[0],undefined,undefined,undefined,pos.top,pos.left);
			}
		},
		mousewheel : function (evt) {
				var 
				s = sa.scpa.settings.perDiv[evt.currentTarget.id],
				rolled = 0,
				direction = 1;
